...
class TransportTask:
    def __init__(self):
        ...
        self.quantityStock = [60, 42, 40, 39, 55]  # Количество товара на складе
        self.quantityOrdered = [20, 28, 32, 16, 10]  # Количество заказанного товара

    def taskOptimization(self):
        # Определение
        self.task = pulp.LpProblem('0', pulp.LpMaximize)
        self.task += (-1 * self.x1 - 4 * self.x2 - 9 * self.x3 - 1 * self.x4 - 7 * self.x5
                      - 3 * self.x6 - 8 * self.x7 - 5 * self.x8 - 1 * self.x9 - 6 * self.x10
                      - 1 * self.x11 - 7 * self.x12 - 9 * self.x13 - 5 * self.x14 - 6 * self.x15
                      - 9 * self.x16 - 5 * self.x17 - 3 * self.x18 - 8 * self.x19 - 4 * self.x20
                      - 7 * self.x21 - 8 * self.x22 - 3 * self.x23 - 3 * self.x24 - 5 * self.x25), "Функция цели"
